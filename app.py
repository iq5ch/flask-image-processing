...
def proses1():
    ...
    if 'otsu' in request.form.get('select_thresholding'):
        ...
        mode = 'sauvola'
    elif 'gaussian_adapt' in request.form.get('select_thresholding'):
        mode = 'gaussian_adapt'
    elif 'blur' in request.form.get('select_thresholding'):
        mode = 'blur'
    elif 'sharpen' in request.form.get('select_thresholding'):
        mode = 'sharpen'
    elif 'canny' in request.form.get('select_thresholding'):
        mode = 'canny'
    ...
    if mode == 'otsu':
        ...
        cv2.imwrite("/".join([target, 'result.jpg']),img_res)
    elif mode == 'gaussian_adapt':
        img_res = process.gaussian_adapt(img)
        cv2.imwrite("/".join([target, 'result.jpg']),img_res)
    elif mode == 'blur':
        img_res = process.img_blur(img)
        cv2.imwrite("/".join([target, 'result.jpg']),img_res)
    elif mode == 'sharpen':
        img_res = process.img_sharpen(img)
        cv2.imwrite("/".join([target, 'result.jpg']),img_res)
    elif mode == 'canny':
        img_res = process.canny_edge(img)
        cv2.imwrite("/".join([target, 'result.jpg']),img_res)
